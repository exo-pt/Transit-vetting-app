...
if __name__ == '__main__':
            ...
            if submit:
                try:
                    with placeholder.container():
                        event = st.plotly_chart(fig, use_container_width=True, selection_mode=('box'), on_select="rerun")
                except:
                    aa=1
    ...
    if (ticid == 0) or (sector == 0):
        ...
        if 'ss_tit' in st.session_state:
            ...
            if len(sres) == 0:
                with placeholder:
                    st.html('<div class="spc"><i>Searching sector '+str(sector)+' TESScut (lightcurve not available)...</i></div>')
                try:
                    sres=lk.search_tesscut(TICstr, sector=sector)
                except:
                    sres=""
